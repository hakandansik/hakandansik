...
from flask import Flask, render_template, request, redirect, flash, jsonify, session, url_for, send_file, send_from_directory, abort
...
# SECRET_KEY ortam değişkeninden alınır; tanımlı değilse hata verir
...
# MongoDB Bağlantısı
# URI ortam değişkeninden okunur; aksi halde yerel sunucuya bağlanılır.
...
def login_required(func=None, *, message="Bu işlemi yapmak için giriş yapmalısınız.", category="warning"):
    """Ensure the user is logged in before calling the view."""
...
@login_required(message="Bu sayfayı görüntülemek için giriş yapmalısınız.")
def sg_form_preview(rez_id):
    if not ObjectId.is_valid(rez_id):
        abort(400, description="Geçersiz rezervasyon ID")
...
def pdf_yukle_ve_kaydet():
    ...
    if os.path.exists(pdf_path):
        ...
        current_app.logger.info("PDF başarıyla base64 olarak MongoDB'ye yüklendi.")
    else:
        current_app.logger.error("PDF dosyası static klasöründe bulunamadı!")
...
def sg_form_tur_preview(tur_id):
    ...
    if not ObjectId.is_valid(tur_id):
        abort(400, description="Geçersiz tur ID")
...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user['password'], password):
            ...
            current_app.logger.info(f"Kullanıcı oturumu açıldı: {session}")
    ...
    if arsivlenecek_kayitlar:
        ...
        try:
            ...
            current_app.logger.info(
                f"{len(arsivlenecek_kayitlar)} kayıt arşiv koleksiyonuna taşındı."
            )
        except Exception as e:
            current_app.logger.error(f"Arşivleme sırasında hata oluştu: {e}")
    else:
        current_app.logger.info("Arşivlenecek geçmiş rezervasyon bulunamadı.")
...
def start_scheduler():
    """Configure and start the daily cleanup scheduler."""
    global scheduler
    ...
    # Yalnızca ana süreçte zamanlayıcıyı başlat
    should_start = False
    werkzeug_run_main = os.environ.get("WERKZEUG_RUN_MAIN", "").lower()
    run_main = os.environ.get("RUN_MAIN", "").lower()
    if werkzeug_run_main == "true" or run_main == "true":
        should_start = True
...
@login_required(message="Lütfen giriş yapın.", category="danger")
def rezervasyon_list():
    try:
        temizle_rezervasyonlar()
    except Exception as e:
        current_app.logger.warning(f"Cleanup at list skip: {e}")
...
def tur_duzenle(tur_id):
    if not ObjectId.is_valid(tur_id):
        abort(400, description="Geçersiz tur ID")
    tur_oid = ObjectId(tur_id)
    tur = tur_collection.find_one({"_id": tur_oid})
    if not tur:
    ...
    if request.method == 'POST':
        ...
        tur_collection.update_one({"_id": tur_oid}, guncelleme)
...
def rezervasyon_duzenle(id):
    ...
    rezervasyon = customer_collection.find_one({"_id": rez_oid})
    current_app.logger.info(rezervasyon)
    ...
    if request.method == 'POST':
        ...
        customer_collection.update_one({"_id": rez_oid}, {"$set": updated_data})
...
def eposta_gonder(bilgiler):
    ...
    if not sender_email or not receiver_email:
        current_app.logger.warning(
            "Gönderen veya alıcı e-posta adresi ortam değişkenlerinde tanımlanmamış."
        )
        return
...
def submit():
    ...
    # Tur bilgisi
    secili_tur = tur_collection.find_one({"_id": tur_oid})
    if not secili_tur:
        flash("Geçersiz bir tur seçildi. Lütfen tekrar deneyin.", "danger")
        return redirect('/')

    # Kontenjan kontrolü
    kayitli_sayisi = customer_collection.count_documents({"TUR_ID": tur_oid})
    ...
    mevcut_rezervasyon = customer_collection.find_one(
        {"TC_PASAPORT": tc_pasaport, "TUR_ID": tur_oid})
...
@app.route('/kontrol_kapasite', methods=['POST'])
def kontrol_kapasite():
    data = request.get_json()
    tur_id = data.get('tur_id') if data else None

    if not tur_id:
        return jsonify({"error": "Gerekli parametre (tur_id) eksik."}), 400

    if not ObjectId.is_valid(tur_id):
        return jsonify({"error": "Geçersiz tur ID"}), 400
...
def odeme():
    ...
    return render_template(
        ...
        sertifika=sertifika,
        tur=tur,
        fiyat=tur.get("FIYAT", 0) if tur else 0,
    )
...
if __name__ == '__main__':
    ...
    app.run(
        ...
        host=os.environ.get('FLASK_HOST', '10.251.3.11'),
